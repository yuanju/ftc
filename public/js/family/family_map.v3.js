var family_map = function (data,queue) {
    ...
    this._init = function () {
        ...
        if((x_max_offset-x_min_offset) < stage_size.width){
            console.log(x_max_offset,x_min_offset);
            this.map_container.x(-this.map_size.width/2+(this.map_size.width-(x_max_offset-x_min_offset))/2);
        }
        if(stage_size.height - y_max_offset > 0){
            var y = y_max_offset < stage_size.height*3/4 ? stage_size.height/4 : stage_size.height - y_max_offset;
            console.log(y_max_offset,stage_size.height*3/4 ,y, stage_size.height, y_max_offset);
            this.map_container.y(-this.map_size.height/2+y);
        }
        //if(this.node_data.length <= 2){
        //    this.map_container.x(-this.map_size.width/2+stage_size.width/2-node_box_size.width/2)
        //        .y(-this.map_size.height/2+stage_size.height/4+node_box_size.height/2);
        //}
    ...
    this.render = function () {
        ...
        offset = {x:0,y:0};
        this.scale_number = scale_object.x;
        var tree_layout_data = this.tree_layout(d3.hierarchy(this.data));
        this.node_data = tree_layout_data.descendants()
        this.link_data = tree_layout_data.links();
        this.link_scale = d3.scaleLinear().domain(d3.extent(this.node_data,function(d){return d.depth;})).range([5,1]);
